...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      '.people-ai-appearance-locked-note{color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:20px;border:1px solid var(--dsw-alias-border-l2);border-radius:8px;background:transparent;padding:3px 10px;white-space:nowrap}',
      ...
      // Cards = the top-level card lists (plugins / models / agent presets).
      // NOTE the selector: li may sit behind a display:contents wrapper
      // (ul > div[data-slot] > li in the plugins section) or be a direct ul
      // child (models / presets), so li is matched as "any li under a
      // top-level ul, itself not inside another li" — that covers both shapes
      // while skipping nested lists inside cards. Buttons = nav cells, row &
      // footer actions. The product's own hover/active fills are overridden,
      // so hover feedback is restored with a brightness lift and the selected
      // state re-applied through the stable a11y hooks (aria-current nav
      // rows, data-active tabs). If any primary action button must keep its
      // brand fill, add it to the :not() list below.
      '[role="dialog"]:has([data-slot="settings.section"]) ul:not(li > ul) li:not(li li),[role="dialog"]:has([data-slot="settings.section"]) button{background:rgba(15,17,21,0.42)!important;-webkit-backdrop-filter:blur(1px);backdrop-filter:blur(1px);box-shadow:0 0 0 1px var(--dsw-alias-border-l3),0 0 1px 0 rgba(0,0,0,0.2),0 4px 12px 0 rgba(0,0,0,0.12),0 12px 32px 0 rgba(0,0,0,0.28)!important}',
      ...
      // Tabs (plugins section: 插件配置/插件列表/用户插件, stable ARIA role) are
      // text-only: no chip box, no fill, no ring — the product's underline and
      // color states stay untouched. Placed after the selected-state rule so the
      // equal-specificity reset also wins for the active tab.
      '[role="dialog"]:has([data-slot="settings.section"]) button[role="tab"]{background:transparent!important;-webkit-backdrop-filter:none!important;backdrop-filter:none!important;box-shadow:none!important}',
      // Card interiors (plugin config cards 终端/Agent循环/网页搜索 etc.): the
      // header / footer buttons inside a card are text-only as well, so each
      // card reads as one outlined surface instead of a box inside a box.
      '[role="dialog"]:has([data-slot="settings.section"]) ul:not(li > ul) li:not(li li) button{background:transparent!important;-webkit-backdrop-filter:none!important;backdrop-filter:none!important;box-shadow:none!important}',
      // The card surface itself becomes transparent too (the 1px frame/ring
      // from the base rule above stays — only the outline separates it).
      '[role="dialog"]:has([data-slot="settings.section"]) ul:not(li > ul) li:not(li li){background:transparent!important}',
      // Inputs inside the settings panel: transparent fill (the product's own
      // border stays, so fields remain visible). Native text-like inputs,
      // textarea and select; checkboxes/radios/hidden/file are left alone.
      '[role="dialog"]:has([data-slot="settings.section"]) input:not([type="checkbox"]):not([type="radio"]):not([type="hidden"]):not([type="file"]),[role="dialog"]:has([data-slot="settings.section"]) textarea,[role="dialog"]:has([data-slot="settings.section"]) select{background:transparent!important}',
      // Sidebar buttons: unified translucent frosted fill (the same 0.42
      // material as the panel/cards, plus a 1px blur; deliberately no ring or
      // shadow — small icon buttons should not read as boxes). The sidebar
      // root is tagged `data-people-ai-sidebar` by JS (below) because it has
      // no stable attribute of its own.
      '[data-people-ai-sidebar] button{background:rgba(15,17,21,0.42)!important;-webkit-backdrop-filter:blur(1px);backdrop-filter:blur(1px)}',
      '[data-people-ai-sidebar] button:hover{filter:brightness(1.18)}',
    ].join('\n');
    ...
    // ── sidebar marker ─────────────────────────────────────────────────────
    // The sidebar root has no stable attribute (hash class only), so the
    // plugin tags it once: walk up from the official settings trigger
    // ([aria-haspopup="dialog"], which lives in the sidebar footer) and mark
    // the first ancestor whose computed background equals the
    // --dsw-specific-sidebar-fill token (that is the sidebar root — the
    // layout column one level up shares the token but is never reached
    // first). A MutationObserver re-tags if the sidebar mounts later or
    // remounts; unload removes the marker.
    const SIDEBAR_MARKER = 'data-people-ai-sidebar';
    let sidebarObserver = null;
    function tagSidebar() {
      if (typeof document === 'undefined') return;
      if (document.querySelector('[' + SIDEBAR_MARKER + ']') !== null) return;
      const trigger = document.querySelector('[aria-haspopup="dialog"]');
      if (trigger === null) return;
      if (typeof getComputedStyle !== 'function') return;
      const token = getComputedStyle(document.body).getPropertyValue('--dsw-specific-sidebar-fill').trim();
      if (token === '') return;
      let el = trigger.parentElement;
      while (el !== null) {
        if (getComputedStyle(el).backgroundColor === token) {
          el.setAttribute(SIDEBAR_MARKER, '');
          if (sidebarObserver !== null) sidebarObserver.disconnect();
          return;
        }
        el = el.parentElement;
      }
    }
    function installSidebarMarker() {
      if (typeof document === 'undefined') return () => {};
      const root = document.body ?? document.documentElement;
      if (root === null) return () => {};
      tagSidebar();
      if (typeof MutationObserver === 'undefined') return () => {};
      sidebarObserver = new MutationObserver(tagSidebar);
      ...
      return () => {
        ...
        const tagged = document.querySelector('[' + SIDEBAR_MARKER + ']');
        if (tagged !== null) tagged.removeAttribute(SIDEBAR_MARKER);
      };
    }
    ...
    function apply(ctx) {
      ...
      // Sidebar buttons get the translucent frosted fill via the JS-tagged
      // marker (registered after the dark-lock effect to keep effect order).
      ctx.effect(installSidebarMarker, 'people-ai: sidebar marker');

      // Semi-transparent tints: the wallpaper glows through the main surface
      // and the sidebar while text stays readable. Wrapped in ctx.effect so
      // the returned disposer is fiber-owned — the override layer is removed
      // on unload instead of leaking (the theme API has no auto-disposal).
      ctx.effect(() => ctx.theme.overrideTokens('people-ai', {
        '--dsw-alias-bg-base': {
